...
if (blockName) {
  ...
  mkdirp(dirPath, (err) => {                                           // создаем
    ...
    if (err) {
      console.error(`[Account] Отмена операции: ${err}`);
    }

    // Нет ошибки, поехали!
    else {
      console.log(`[Account] Создание папки ${dirPath} (если отсутствует)`);
      ...
      extensions.forEach((extention) => {
        ...
        if (extention === 'scss') {
          ...
          if (!hasThisBlock) {
            ...
            fileCreateMsg = '[Account] Подключение блока добавлено в projectConfig.json';
          }
        }
        ...
        // Если нужна подпапка для картинок
          ...
          if (fileExist(imgFolder) === false) {
            mkdirp(imgFolder, (err) => {
              ...
              else console.log(`[Account] Создание папки: ${imgFolder} (если отсутствует)`);
            });
          } else {
            console.log(`[Account] Папка ${imgFolder} НЕ создана (уже существует) `);
          }
        }
        ...
        if (fileExist(filePath) === false && extention !== 'img') {
          fs.writeFile(filePath, fileContent, (err) => {
            if (err) {
              return console.log(`[Account] Файл НЕ создан: ${err}`);
            }
            console.log(`[Account] Файл создан: ${filePath}`);
            if (fileCreateMsg) {
              console.warn(fileCreateMsg);
            }
          });
        } else if (extention !== 'img') {
          console.log(`[Account] Файл НЕ создан: ${filePath} (уже существует)`);
        }
      });
    }
  });
} else {
  console.log('[Account] Отмена операции: не указан блок');
}
